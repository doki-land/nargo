/**
 * Placeholder / Trusted Publisher helpers for Nargo.
 *
 * First publish (0.0.0) cannot use `nargo release` bootstrap — park names here.
 *
 * Commands:
 ...
 *
 * Real packages later: git tag v* → CI OIDC (Trusted Publisher) + provenance.
 *
...
/** Keep in sync with runtimes/* publish set (facade + @nargo/* + platform .node). */
const JS_STUBS = [
    "@nargo/nargo",
    "@nargo/core",
    "@nargo/testing",
...
/** Platform NAPI packages (optionalDependencies of `nargo`). */
const NATIVE_STUBS = [
    { name: "@nargo/nargo-win32-x64", os: ["win32"], cpu: ["x64"] },
    { name: "@nargo/nargo-darwin-x64", os: ["darwin"], cpu: ["x64"] },
    { name: "@nargo/nargo-darwin-arm64", os: ["darwin"], cpu: ["arm64"] },
    { name: "@nargo/nargo-linux-x64", os: ["linux"], cpu: ["x64"] },
    { name: "@nargo/nargo-linux-arm64", os: ["linux"], cpu: ["arm64"] },
];

/** Stub reservation version (name park only). */
const VERSION = "0.0.0";

/** Trusted Publisher expect — sole npm trusted workflow. */
const TRUST = {
    repo: "doki-land/nargo",
...
// Trust needs a 2FA-capable session (~/.npmrc). Do not let a dead granular
// NPM_TOKEN in .env override it — only honor --token / process.env for trust.
// Publish stubs may still use localEnv.NPM_TOKEN.
...
const STUBS = [
    ...
    ...NATIVE_STUBS.map((s) => ({
        ...s,
        description: `Optional NAPI binding (${s.name}). Placeholder only.`,
    })),
];
...
function runNpm(args, opts = {}) {
    ...
    let userConfig;
    if (token) {
        userConfig = path.join(os.tmpdir(), `nargo-placeholder-npmrc-${process.pid}`);
...
function writeStub(spec, dir, authToken) {
    ...
    const pkg = {
        name: spec.name,
        version: VERSION,
        description: spec.description ?? "Nargo placeholder — not for production use.",
        license: "MIT",
        private: false,
        files: ["README.md"],
    };
    ...
    fs.writeFileSync(
        ...
        `# ${spec.name}\n\nPlaceholder package (${VERSION}). Reserved for Nargo (nargo / @nargo/*).\n`,
    );
    if (authToken) {
...
function cmdPublish() {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nargo-npm-stubs-"));
