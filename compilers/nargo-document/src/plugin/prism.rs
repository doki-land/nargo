//! Prism 代码高亮插件
//!
//! 提供对 Markdown 中代码块的 Prism 语法高亮支持
...
impl PrismPlugin {
    ...
    /// 处理代码块，将 ```lang ... ``` 替换为 <pre class="language-lang"><code class="language-lang">...</code></pre>
    ///
    /// # Arguments
    ///
    /// * `content` - 包含代码块的文本内容
    ...
    pub fn process_code_blocks(&self, content: &str) -> String {
        CODE_BLOCK_RE
            .replace_all(content, |caps: &regex::Captures| {
                ...
                let code = &caps[2];
                if lang.is_empty() { format!("<pre><code>{}</code></pre>", code.trim()) } else { format!("<pre class=\"language-{}\"><code class=\"language-{}\">{}</code></pre>", lang, lang, code.trim()) }
            })
            .to_string()
    }
}
