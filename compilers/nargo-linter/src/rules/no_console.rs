    ...
    fn check_expr(&self, expr: &JsExpr, diagnostics: &mut Vec<Diagnostic>) {
        match expr {
            JsExpr::Call { callee, args, .. } => {
                if let JsExpr::Member { object, property, computed, .. } = callee.as_ref() {
                    if !*computed {
                        if let JsExpr::Identifier(name, _, _) = object.as_ref() {
                            if let JsExpr::Identifier(prop, _, _) = property.as_ref() {
                                if name == "console" && prop == "log" {
                                    diagnostics.push(Diagnostic { code: self.name().to_string(), message: "Unexpected 'console.log'".to_string(), severity: Severity::Warning, line: 1, column: 1 });
                                }
                            }
                        }
                    }
                            ...
                            if let JsExpr::Identifier(prop, _, _) = property.as_ref() {
                                if name == "console" && prop == "log" {
                                    ...
                                    if start < end && end <= source.len() {
                                        return Some(FixAction { start, end, replacement: "".to_string(), description: "Remove console.log".to_string() });
                                    }
                                }
                            }
                        }
